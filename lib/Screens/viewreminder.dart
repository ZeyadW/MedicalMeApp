...
class _ViewReminderState extends State<ViewReminder> {
  ...
  void initState() {
    ...
    if (reminder.type == _dropdownMenuItems[2].value.name)
      _rtypeItem = _dropdownMenuItems[2].value;
  ...
  void _updatedays(reminder, id) {
    _db.collection("reminders").doc(id).update({
      "days": daysarray,
    });
  }
